...
#ifndef NUSSL_STRING_H
#define NUSSL_STRING_H
 ...
 * character was found.
 *
 * nussl_qtoken will return NULL if unterminated quotes are found. */
char *nussl_token(char **str, char sep);
char *nussl_qtoken(char **str, char sep, const char *quotes);
...
char *nussl_base64(const unsigned char *text, size_t len);
 ...
 * allocated space. */
nussl_buffer *nussl_buffer_ncreate(size_t size);

/* Returns size of data in buffer, equiv to strlen(nussl_buffer_data(buf)) */
#define nussl_buffer_size(buf) ((buf)->used - 1)
 ...
 * 'buf'. */
#define nussl_buffer_czappend(buf, str) \
nussl_buffer_append((buf), (str), sizeof((str)) - 1)
 ...
 * 'buffer'. */
char *nussl_strerror(int errnum, char *buffer, size_t buflen);

/* nussl_strnzcpy copies at most 'n'-1 bytes of 'src' to 'dest', and
 * ensures that 'dest' is subsequently NUL-terminated. */
#define nussl_strnzcpy(dest, src, n) do { size_t nussl__nm1 = (n) - 1;      \
strncpy(dest, src, nussl__nm1); dest[nussl__nm1] = '\0'; } while (0)
 ...
 * char. */
#define nussl_tolower(c) (nussl_tolower_array()[(unsigned char)c])
...
#define NUSSL_ASC2HEX(x) (((x) <= '9') ? ((x) - '0') : \
                       (nussl_tolower((x)) + 10 - 'a'))
