...
G_MODULE_EXPORT gint user_packet_logs (connection element, int state){
      ...
      case STATE_OPEN:
        switch ((element.tracking_hdrs).protocol){
          case IPPROTO_TCP:
            ...
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,tcp_sport,tcp_dport,start_timestamp,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%u,%u,%lu,'ACCEPT');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
          ...
          case IPPROTO_UDP:
            ...
            strncpy(tmp_inet1,inet_ntoa(ipone),40) ;
            strncpy(tmp_inet2,inet_ntoa(iptwo),40) ;
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,udp_sport,udp_dport,start_timestamp,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%u,%u,%lu,'ACCEPT');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
                  (element.tracking_hdrs).protocol,
          ...
          default:
            ...
            strncpy(tmp_inet1,inet_ntoa(ipone),40) ;
            strncpy(tmp_inet2,inet_ntoa(iptwo),40) ;
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,start_timestamp,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%lu,'ACCEPT');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
                  (element.tracking_hdrs).protocol,
      ...
      case STATE_DROP:
        switch ((element.tracking_hdrs).protocol) {
          case IPPROTO_TCP:
            ...
            strncpy(tmp_inet1,inet_ntoa(ipone),40) ;
            strncpy(tmp_inet2,inet_ntoa(iptwo),40) ;
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,tcp_sport,tcp_dport,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%u,%u,'DROP');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
                  (element.tracking_hdrs).protocol,
   ...
   ipone.s_addr=ntohl((element.tracking_hdrs).saddr);
            ...
            strncpy(tmp_inet1,inet_ntoa(ipone),40) ;
            strncpy(tmp_inet2,inet_ntoa(iptwo),40) ;
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,udp_sport,udp_dport,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%u,%u,'DROP');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
                  (element.tracking_hdrs).protocol,
          ...
          default:
            ...
            strncpy(tmp_inet1,inet_ntoa(ipone),40) ;
            strncpy(tmp_inet2,inet_ntoa(iptwo),40) ;
            if (snprintf(request,511,"INSERT INTO %s (user_id,oob_time_sec,ip_protocol,ip_saddr,ip_daddr,oob_prefix) VALUES (%u,%lu,%u,'%s','%s',%lu,'DROP');",
                  pgsql_table_name,
                  (element.user_id),
                  element.timestamp,
                  (element.tracking_hdrs).protocol,
